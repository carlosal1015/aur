...
ARG CORE_PACKAGE="boost"

ARG PATCH="https://gitlab.archlinux.org/carlosal1015/boost/-/commit/1ad85c5dba6fcacc1a04122bc6d7a2e11d96733c.patch"
...
RUN sudo pacman-key --init && \
  ...
  find /tmp/ ! -name '*.log' ! -name 'boost-*.pkg.tar.zst' -type f -exec rm -f {} + && \
  sudo pacman -Scc <<< Y <<< Y && \
